...
void Blob::drawBlob(){
    ...
    for(int i=0;i<(int)_all_particules.size();i++){
        if((i%_width+1)<4 && (i+1+_width*_depth)<_all_particules.size() && (i+_width*_depth)<_all_particules.size()){
        ...
        if((i%16+4)<_width*_depth && (i+4+_width*_depth)<_all_particules.size() && (i+_width*_depth)<_all_particules.size()){
